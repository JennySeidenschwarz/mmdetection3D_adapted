...
def load(file,
    ...
    if filter_list:
        ...
        with open(f'new_seq_splits_Waymo_Converted_fixed_val/{percentage}_{detection_type}.txt', 'r') as f:
            seqs = f.read()
            seqs = seqs.split('\n')
        seqs.append('16473613811052081539')
        waymo2kitti = pd.read_csv(f'logid_2_kitti/{prefix}_naming.csv')
        waymo2kitti['path'] = waymo2kitti['log_id']
        waymo2kitti['log_id'] = waymo2kitti['log_id'].astype(str)
    ...
    if filter_list or all_car: 
        ...
        for d in obj['data_list']:
            if filter_list and d['sample_idx'] not in waymo2kitti['whole_name'].values.tolist():
                continue
            if all_car:
                for i in range(len(d['instances'])):
                    d['instances'][i]['bbox_label_3d'] = 0
                    d['instances'][i]['bbox_label'] = 0
...
if __name__ == "__main__":
    load("/workspace/waymo_kitti_format_annotaions/kitti_format/waymo_infos_val.pkl")
